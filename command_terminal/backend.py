...
def intelligent_offline_ai(query: str) -> str:
    ...
    if "create a folder" in query or "make a folder" in query:
        match = re.search(r'folder (?:called |named )?([\w.-]+)', query)
        if match:
    ...
    if "delete file" in query or "remove file" in query:
        ...
        if match: commands.append(f"{del_file_cmd} {match.group(1)}")
    if "delete folder" in query or "remove directory" in query:
        match = re.search(r'(?:delete|remove) (?:folder|directory) ([\w.-]+)', query)
        if match: commands.append(f"{del_folder_cmd} {match.group(1)}")
    if "copy file" in query:
        match = re.search(r'copy file ([\w.-]+) to ([\w.-]+)', query)
        if match: commands.append(f"{copy_cmd} {match.group(1)} {match.group(2)}")
    if "read file" in query or "show content of" in query:
        match = re.search(r'file ([\w.-]+)', query)
        if match: commands.append(f"{read_cmd} {match.group(1)}")
    ...
    if commands:
        ...
        return f'echo "Offline AI: I couldn\'t understand that."'

def translate_nl_to_command(query: str) -> str:
    if is_openai_configured():
        try:
            logging.info(f"Translating with live OpenAI: '{query}'")
            system_prompt = f"You translate natural language to a shell command for a {platform.system()} environment. Return only the command."
            response = openai.chat.completions.create( model="gpt-3.5-turbo", messages=[ {"role": "system", "content": system_prompt}, {"role": "user", "content": query} ] )
            return response.choices[0].message.content.strip()
        except Exception as e:
            logging.error(f"OpenAI API call failed: {e}. Falling back to Offline AI.")
            emit('command_error', f"Live AI failed. Using Offline AI.")
...
def handle_connect():
    ...
    emit('command_output', f"Welcome! Directory: {user_session['current_directory']}")
